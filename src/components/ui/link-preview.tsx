...
export function LinkPreview({ 
  ...
  useEffect(() => {
    // In a real implementation, you would call your backend API to fetch 
    // the metadata (to avoid CORS issues)
    const fetchPreview = async () => {
      ...
      try {
        ...
        // Demo preview data - in production, you'd get this from an API
        setPreviewData({
          title: `Content from ${domain}`,
          description: 'This is a preview of the linked content. In a real implementation, this would be extracted from the page metadata.',
          image: domain.includes('github') 
            ? 'https://github.githubassets.com/images/modules/site/social-cards/github-social.png'
            : domain.includes('twitter') || domain.includes('x.com')
            ? 'https://abs.twimg.com/responsive-web/client-web/icon-default.77d93ffb.png'
            : null,
  ...
  if (error) {
    return (
      <motion.div
        ...
        className={cn(
          "overflow-hidden rounded-md border border-white/10 bg-white/[0.02] backdrop-blur-md",
          "flex items-center",
          className
        )}
      >
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="p-3 text-sm text-white/80 hover:text-white transition-colors flex items-center gap-2"
        >
  ...
  if (loading) {
    return (
      <motion.div
        ...
        exit={{ opacity: 0 }}
        className={cn(
          "overflow-hidden rounded-md border border-white/10 bg-white/[0.02] backdrop-blur-md p-3",
          "flex items-center justify-center",
          className
        )}
      >
        <Loader2 className="w-4 h-4 text-white/60 animate-spin" />
        <span className="ml-2 text-sm text-white/60">Loading preview...</span>
      </motion.div>
    );
  }
  ...
  if (compact) {
    return (
      ...
      >
        {previewData.favicon && (
          <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center ml-2">
            <img 
        ...
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="p-3 text-sm text-white/80 hover:text-white transition-colors flex-1 truncate"
        >
          {previewData.title}
        </a>
        {onClose && (
    ...
    >
      ...
      <div className="p-4 flex flex-col gap-2">
        <div className="flex items-start justify-between">
          ...
          {onClose && (
            <button 
              type="button"
              onClick={onClose}
              className="p-1 text-white/50 hover:text-white/90 transition-colors -mt-1 -mr-1"
            >
              <X size={16} />
            </button>
          )}
        ...
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          className="mt-2 text-xs text-white/60 hover:text-white/90 transition-colors flex items-center gap-1.5"
        >
